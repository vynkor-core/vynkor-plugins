...
- **`UpdateStream::next()` (grammers) survives its own errors** — `&mut self`, internal `message_box` state intact after an `Err`. The listener loop in `events/mod.rs` relies on this: on error it backs off (1s→30s exponential) and retries the *same* stream forever. Don't reintroduce a `break` on error — that silently kills live updates for the rest of the process's life with no recovery.
